...
export interface ModuleManifest {
   ...
   *
   *  A CONFORMANCE GATE ON THIS FIELD IS ON FOR finish AND speech (core#223). `checkManifest` FAILS
   *  a module serving a hook in `CEILING_DERIVED_HOOKS` that omits it. Load stays permissive:
   *  `validateManifest` still LOADS an absent value, so a third-party module is not our gate to
   *  fail. Deliberately not a warning-with-override: an override that becomes routine is
   *  indistinguishable from the check being off.
   *
