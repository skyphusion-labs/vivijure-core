...
client) instead of raw `curl` or the browser. The MCP ships in **`@skyphusion-labs/vivijure-core`**
(`src/mcp.ts`) and deploys as a **separate** Worker or service from the studio core. Host repos
(`vivijure-cf`, `vivijure-local`) supply only wrangler/deploy config (`wrangler.mcp.toml`). It is a
thin proxy: your agent authenticates with `MCP_TOKEN`, and each tool call forwards to the studio API.
...
Package paths (import as `@skyphusion-labs/vivijure-core/mcp`, etc.):
