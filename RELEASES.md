...
>
> **CORRECTED AT THE v1.16.0 CUT (rollins, 2026-08-15): NOTHING FORCES THAT BUMP ANY MORE, and the
> paragraph above describes an obligation with no mechanism behind it.** core#224's `changelog.d/`
> fragments removed the need for a feature PR to touch `CHANGELOG.md` at all, and touching
> `CHANGELOG.md` was the only moment at which core#146's guard could see the version. A
> fragment-only PR is post-release-main shaped -- top heading and `package.json` agree on the spent
> version -- so it passes `tests/changelog-version.test.ts` green while opening no cycle. Measured
> on this cut: both cycle PRs (#233, #235) used fragments, neither bumped, and
> `changelog-assemble.mjs` then **refused** with *"does not open with an Unreleased-shaped heading.
> Nothing to promote"*.
>
...
|---|---|---|---|---|
| `vivijure-core-v1.21.8` | 1.21.8 | 8dc0858 | 2026-08-18 | **PATCH.** Cast voice sample rides motion as `voice_ref_url` / `voice_ref_keys`. Seedance is the exact lock; Veo has no such field. Hosts must apply the `voice_ref_key` D1 column before pinning. |
...
| `vivijure-core-v1.17.0` | 1.17.0 |  |  | **MINOR.** `callVideoFinish` / `callAudioMix` / `callVideoFinishInspect` send `Authorization: Bearer` when `MEDIA_FINISH_TOKEN` (or `FINISH_DOOR_TOKEN`) is readable, so arming the fleet media container token (vivijure-cf#613) does not 401 assemble and mux. Unset stays fail-open. |
| `vivijure-core-v1.16.0` | 1.16.0 |  |  | **MINOR.** The RunPod job log MOVES into core as the new public subpath `@skyphusion-labs/vivijure-core/runpod-job-log`, and cast LoRA training records through it (vivijure-cf#475). Cast training was the one GPU path in the estate with no job record at all: the `vivijure-wan-train` endpoint billed 14.5% of GPU spend on 2026-08-01 and 21.9% on 2026-08-02 with ZERO rows in `runpod_job_log` on either day -- absent rather than mis-attributed, and absent in the flattering direction. The recorder had to MOVE rather than be copied, because `recordRunpodJob` lived in `vivijure-cf/modules/_shared/` and core sits UPSTREAM of it, so no call site written in core could ever have reached it; same move and same reasoning as v1.10.0 made for `runpod-route.ts` (cp#321). Also `RunpodJobBackend` and an optional `backend` tag on `RunpodResult`, set by the function that MAKES the routing choice, plus `castTrainJobLogModule` and the two `CAST_TRAIN_*_JOB_LOG_MODULE` labels. All additive; a caller that reads none of it gets byte-identical behaviour except that a RunPod-backed cast train now writes a row. Carries core#232 (scan-cache comment). **CONSUMER CONDITION, `vivijure-local` ONLY:** its `runpod_job_log` has SEVEN columns and core's upsert binds NINE (no equivalent of vivijure-cf `migrations/0019_runpod_job_log_timing.sql`), so repinning local to 1.16.0 before that migration makes every cast-train job-log write fail. It fails SOFT -- the recorder never throws and never fails a render -- so the symptom is a warn and an empty table, which is the same shape cf#475 was about. Migration first: vivijure-local#415. `vivijure-cf` is unaffected (its table already has the columns) and repins in vivijure-cf#610. |
| `vivijure-core-v1.15.0` | 1.15.0 | e074b2a | 2026-08-15 | **MINOR.** Carries core#216's cache seam -- the `MODULE_*` service scan is cached 30s while the D1 dispatch set is re-read on every call, and an incomplete scan is never cached. That fix could not reach a panel without this publish: `vivijure-cf` pins `^1.14.0` with a lockfile resolving 1.14.0 and release running `npm ci`, so the caret alone changes nothing. Also core#182 (the 90-minute phase ceiling becomes a FLOOR, sized from declared module ceilings; enforcement deliberately sequenced to core#223 with the suite asserting `checkManifest` stays SILENT so that landing is a visible change), core#205 (`output_json` single-sourced across FIVE writers, not the three the issue named, and `keyframes_incomplete` was a live shipping defect found on the way), core#215 (`env.DB.batch` guarded -- the unguarded call let a batch-less host report a successful submit having written 0 of 3 shard rows), and the changelog machinery: core#207's ancestry guard, core#224's `changelog.d/` fragments, core#228's release-cut guard. |
| `vivijure-core-v1.14.0` | 1.14.0 | 138fc25 | 2026-08-14 | **MINOR.** Per-render participation for the `finish` chain (cf#537). `finish` is a CHAIN hook, so BINDING a module was the entire enrolment and every bound finish module ran on every shot -- `finish-blender` was applying a real `filmic_warm` grade at strength 1.0 to every film, unrequested, while the job reported `done, failed 0, degraded 0`. Adds `ModuleManifest.participation` (`default` | `opt_in`, OPTIONAL and additive, no `MODULE_API` bump), the `HookSelection` tagged union, `selectForChain`, and `SELECTABLE_HOOKS` (**`finish` only** -- the other six chain hooks keep run-every-bound-module, deliberately). The selection rides INSIDE `renderOverrides` so replay paths and un-updated native clients inherit it, lands on `FilmJob.finish_select` and is part of BOTH idempotency natural keys. **A caller that sends nothing is unchanged except that an `opt_in` module no longer runs unasked**, so there is no flag day. Consumers repin to pick it up. |
...
| `vivijure-core-v1.2.12` | 1.2.12 | c761e2c | 2026-07-23 | **K3 closeout (core#89).** Reject unsafe GPU `output_key` in `updateRenderFromView`. |
...
| `vivijure-core-v1.2.9` | 1.2.9 | e734eb2 | 2026-07-23 | **KF3 cast train family parse (core#84).** Honor `wanConfigured` when train fields live only in `renderOverrides`. |
| `vivijure-core-v1.2.8` | 1.2.8 | d7d3ee2 | 2026-07-23 | **Default cast `/train-lora` to Wan train EP when wired (cf#29 Phase E).** SDXL escape via `model_family:"sdxl"`. Tag predates #84 merge; npm 1.2.8 matches tag commit before KF3 fix. |
...
| `vivijure-core-v1.2.3` | 1.2.3 | d01ee29 | 2026-07-21 | **#53 advanceFilmJob wedge -> terminal fail** (core#64) + **COMPLETED renders stamp `output_key`** when `film.mp4` exists (local#99, core#65). |
