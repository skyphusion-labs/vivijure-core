...
|---|---|---|---|---|
| `vivijure-core-v1.8.0` | 1.8.0 |  |  | **MINOR: open version.** Seeded by the version-bump PR (#159) per the SEEDED/CLOSED procedure above; `source commit` and `published` stay EMPTY until the tag is pushed and npm confirms. Opened so `## Unreleased` entries are legal against an untagged version -- 1.7.3 was already tagged, which made the core#146 guard reject main the moment any Unreleased entry landed. Contents are whatever merges before the tag is cut; this row does not claim a scope. |
